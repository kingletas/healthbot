...
A healthy stack exits 0 with no alert. The `feeder` service keeps CloudWatch datapoints fresh, because the bot reads the newest datapoint of the last five minutes and a stale seed means an empty `aws_metrics`. It runs the bot's own `healthbot-local feed` once a minute, so there is no manual step between seeding and running. Run `healthbot-local feed` by hand only when the stack is down and you want datapoints anyway.

Mattermost is at <http://localhost:8065>, as `sre@local.test` / `SuperSecret-123`, bootstrapped by the stub container (first user, team `sre`, incoming webhook into `~town-square`). Grafana and Prometheus are the observability stack's usual `:3000` and `:9090`.
...
- **Mattermost here is still HealthBot's own**, and it is deliberately *not* migrated to the shared one, because the stub container bootstraps it with specific credentials and a `~town-square` channel. It binds `127.0.0.1:8065` rather than `0.0.0.0:8065` so it can run beside the shared Mattermost on `172.17.0.1:8065`; a `0.0.0.0` bind covers the bridge address too, which is why the two used to be mutually exclusive.
