...
the default run; needs:
    ...
    docker compose -f ~/Services/dev-services/docker-compose.yaml up -d
...
def mattermost_posts() -> list:
    ...
    login = requests.post(
        ...
        json={"login_id": "sre@local.test", "password": "SuperSecret-123"},
        timeout=10,
    )
...
def test_staged_outage_pages_into_mattermost():
    ...
    the run must alert: exit 0 — the *site* is degraded, not the monitor —
    ...
    # Assert real rendered content, not just the bot's name — the shim's
...
def test_sns_alert_lands_on_the_local_topic():
    ...
    covered by the exit code — MiniStack accepts the publish).
    """
    import boto3
