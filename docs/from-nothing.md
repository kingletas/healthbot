...
APIs, and an alert landing in a chat client — with no AWS account and no
...
Then it folds in what the other tools already know — response times from New
Relic, active users from Google Analytics, instance and database metrics from
CloudWatch — and decides whether anyone needs waking up.
...
Give Mattermost a minute on its first start — it migrates its own schema before
...
Run the bot again. It still exits 0 — the run itself worked, and that is the
distinction that matters: a non-zero exit means HealthBot broke, not that your
...
The thresholds below those — `alert_limit`, `app_response_alert`,
...
Everything else — New Relic keys, Google Analytics credentials, Slack and Twilio
tokens — comes from AWS Secrets Manager at run time and is never cached. The
