"""
Searches for a product, adds it to cart and loads checkout, outside-in and
...
scaffolding, and a failure now leaves evidence instead of a bare False: a
screenshot, the page HTML and a replayable trace under logs/.
"""
...
def validate_checkout(
    base_url: str, search_term: str | None = None, evidence_dir: str | None = None
) -> bool:
    # A browser that fails to launch must report checkout as down, not
    # silently skip the check, because "cannot tell" pages.
    try:
