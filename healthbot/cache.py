...
Every reader here has a source of truth behind it, whether a packaged YAML
file, Parameter Store or the EC2 API, so a Redis that is down or slow costs a
...
class Cache:
    ...
    def set(self, key, value, duration: int = 0):
        ...
        try:
            return self.cache.set(prepared, bytes(value), ex=timedelta(minutes=duration))
