...
- [x] ~~**Give the KMS key an explicit policy.**~~ The library module now builds it: the role running the deploy, plus anything in `kms_admin_arns`, administers the key, and only the instance role may use it. Account root no longer delegates to IAM. Proved against MiniStack as an in-place policy update on the same key; we need more testing against a real account, where the deploying role rather than root becomes the admin.
