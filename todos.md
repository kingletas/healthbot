...
- [ ] **The cloud-init payload is base64-encoded twice and very likely never runs.** `data.cloudinit_config.this` sets `base64_encode = true`, and the provider base64-encodes whatever reaches `user_data` again before the API call, so the instance is handed a blob cloud-init will not decode: it handles gzip, not base64. If so, `/etc/healthbot.env` is never written on a fresh instance. **Do not fix it by switching to `user_data_base64`** — `user_data` has a StateFunc that hashes the value, and `user_data_base64` does not, so that swap trades a boot defect for the payload sitting in state in clear. No scanner reports either, and the emulator accepts user data without booting anything, so a local plan cannot catch it.
